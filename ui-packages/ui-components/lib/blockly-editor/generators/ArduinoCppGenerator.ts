...
export class ArduinoCppGenerator extends Blockly.CodeGenerator {
  ...
  constructor() {
    ...
    // When a variable is a procedure parameter, skip the global declaration —
    // the parameter already appears in the function signature.
    this.forBlock['variables_get_dynamic'] = (
      ...
      if (!this.paramVarIds_.has(varId)) {
    ...
    // Parameter types are read from the VariableModel; untyped variables
    // default to `int`. Return type for procedures_defreturn is `int`.
    ...
    const buildDefinition = (
      ...
      const paramList = varModels
        .map((model) => {
          const varName = generator.getVariableName(model.getId());
  ...
  override init(workspace: Blockly.Workspace): void {
    ...
    // variables blocks expose via getVarModels(); FieldTypedParamInput owns its
    // variable but doesn't expose it, so getVariableName(varId) would fall
    // through to sanitising the raw varId (yielding garbage like
    // "N4_25qe__JZtrhUA8tSb_25z") whenever the param isn't referenced by a
    // variables_get_dynamic block inside BODY.
