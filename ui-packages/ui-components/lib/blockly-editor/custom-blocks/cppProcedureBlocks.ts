...
// ── Helper: find argId for a minus-button click ─────────────────────────────

function argIdFromMinusField(
  block: Blockly.Block,
  minusField: Blockly.Field,
): string | null {
...
function registerDefBlock(type: string, hasReturn: boolean): void {
  ...
  Blockly.Blocks[type] = {
    ...
    plus(this: CppProcedureDefBlock): void {
      this.addArg_();
      // Defer mutateCallers to avoid interfering with Blockly's drag system
      // during the field click event.
    ...
    addArg_(this: CppProcedureDefBlock, argId?: string, value?: string): void {
      ...
      // Keep STACK (and RETURN) at the bottom.
...
export function initCppProcedureFlyout(workspace: Blockly.WorkspaceSvg): void {
  workspace.registerToolboxCategoryCallback(
    ...
    (ws: Blockly.WorkspaceSvg): Blockly.utils.toolbox.FlyoutItemInfoArray => {
      ...
      // Gather existing cpp procedure definitions.
      ...
      // Caller blocks for no-return procedures.
      ...
      // Caller blocks for return procedures.
