...
// SVG icons extracted from @blockly/block-plus-minus bundle (v9.0.10).
// The package doesn't export createPlusField / createMinusField, so we
// replicate the exact same FieldImage + mutation-event pattern locally.
...
export function getMutationState(block: Blockly.Block): string {
...
// Creates a FieldImage that on click: calls block.plus(), fires BlockChange mutation event.
