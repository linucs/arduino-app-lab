 ...
 *
 * Used for function/callback parameter name slots (Bridge.provide, hat-style
 * catalog blocks, and C++ procedure parameter rows).
 *
 * Unlike FieldVariable (dropdown picker), this field looks and behaves like a
 * plain text input — the user types a name, introducing a new local parameter.
 * Internally it creates a matching workspace variable so that
 ...
 *   dispose() — deletes the variable if no other field or block references it
 */
